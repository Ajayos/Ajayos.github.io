...
function makeDir(path, callback) {fs.mkdir(path, (err) => {if (err) {fs.stat(path, (err2, stat) => {if (err2 || !stat.isDirectory()) {callback(err);} else {callback();}});} else {callback();}});};
...
function getUser(cwd) {
    ...
    ]).then((results) => {return {name: results[0].output.trim(), email: results[1].output.trim()};}).catch((err) => {return null;});
};


module.exports = {
    uniqueDirs: uniqueDirs,
    byShortPath: byShortPath,
